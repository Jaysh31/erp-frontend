...
export default function PurchaseOrderForm() {
  ...
  useEffect(() => {
    if (isEdit && id) {
      ...
      // Generate new PO number for create mode
  ...
  const handleSubmit = async () => {
    ...
    // Prepare payload with ID first for edit
    const payload: any = {
      name: formData.poNumber,
      naming_series: "PO-.YYYY.-",
      supplier: formData.supplierCode || "SUP-00001",
    ...
    try {
      let response;
      if (isEdit && id) {
        // For edit - PUT request with ID inside payload
