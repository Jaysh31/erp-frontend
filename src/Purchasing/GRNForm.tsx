...
export default function GRNForm() {
  ...
  const fetchWarehouses = async () => {
    ...
    try {
      ...
      if (response.data && response.data.success === 1 && response.data.data?.records?.length) {
        setWarehouses(response.data.data.records);
      } else {
        setWarehouses([
          { id: 1, warehouse_name: "Raw Material Store", company: "", parent_warehouse: null, warehouse_type: "Raw Material", city: null, state: null, email_id: null, phone_no: null, disabled: 0 },
          { id: 2, warehouse_name: "Work in Progress", company: "", parent_warehouse: null, warehouse_type: "Work in Progress", city: null, state: null, email_id: null, phone_no: null, disabled: 0 },
          { id: 3, warehouse_name: "Finished Goods", company: "", parent_warehouse: null, warehouse_type: "Finished Goods", city: null, state: null, email_id: null, phone_no: null, disabled: 0 },
        ]);
      }
    } catch (err) {
      console.warn('Notice: Using default warehouses in GRN form:', err);
      setWarehouses([
        { id: 1, warehouse_name: "Raw Material Store", company: "", parent_warehouse: null, warehouse_type: "Raw Material", city: null, state: null, email_id: null, phone_no: null, disabled: 0 },
        { id: 2, warehouse_name: "Work in Progress", company: "", parent_warehouse: null, warehouse_type: "Work in Progress", city: null, state: null, email_id: null, phone_no: null, disabled: 0 },
        { id: 3, warehouse_name: "Finished Goods", company: "", parent_warehouse: null, warehouse_type: "Finished Goods", city: null, state: null, email_id: null, phone_no: null, disabled: 0 },
                    ...
                    <table className="pif-inline-table grnf-items-table pof-inline-table">
                      <thead>
                        <tr>
                          ...
                          <th className="pif-ith">Item Code <span className="grnf-required">*</span></th>
                          <th className="pif-ith">Item Name <span className="grnf-required">*</span></th>
                          <th className="pif-ith">HSN</th>
                          <th className="pif-ith pif-ith-num">Ordered Qty</th>
                          <th className="pif-ith pif-ith-num">Received Qty <span className="grnf-required">*</span></th>
                          <th className="pif-ith pif-ith-num">Rejected Qty</th>
                          <th className="pif-ith">UOM</th>
                          <th className="pif-ith pif-ith-num">Rate <span className="grnf-required">*</span></th>
                          <th className="pif-ith pif-ith-num">Amount</th>
                          <th className="pif-ith pif-ith-num">Tax% <span className="grnf-required">*</span></th>
                        ...
                        {formData.items.map((item, index) => {
                          ...
                          return (
                            <tr key={item.id} className="pif-itr pof-itr">
                                      ...
                                      <div className="pif-dropdown-down">
                                        {suggestionsList.length > 0 ? (
                                            ...
                                            {suggestionsList.map((suggestion) => (
                                              ...
                                              >
                                                <div className="pif-dropdown-item-code">
                                                  <span className="pif-item-code-highlight">{suggestion.item_code || ''}</span>
                                                </div>
                                                <div className="pif-dropdown-item-name">{suggestion.item_name || ''}</div>
                                                <div className="pif-dropdown-item-details">
                                                  <span className="pif-item-rate">₹{(suggestion.standard_rate || suggestion.valuation_rate || 0).toFixed(2)}</span>
                                                  <span className="pif-item-uom">{suggestion.stock_uom || 'NOS'}</span>
                                                  {suggestion.item_group && <span className="pif-item-group">{suggestion.item_group}</span>}
                                                </div>
                                              </li>
                                            ))}
                                          </ul>
                                          ...
                                          <div className="pif-dropdown-empty">
                                            {loadingItemsMaster ? 'Loading items...' : (itemSearch || searchTerms[index] ? 'No items found' : 'Type to search items...')}
                                          </div>
                                        )}
                                      </div>
                                    </div>
                              ...
                              <td className="pif-itd pof-itd pif-itd-num" data-label="Rate *">
                                <input
                                  type="number"
                                  className="pif-cell-input pof-cell-input pif-cell-number"
